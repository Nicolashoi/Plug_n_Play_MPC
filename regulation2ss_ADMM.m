function u0 = regulation2ss_ADMM(x0, N, param, xs, us, Qi, Ri)
    ...
    %% Constraints: Outer loop over subsystems, inner loop over Horizon
        for i=param.activeDGU % loop over all subsystems
          ...
          [w_Ni{i,k}, vi{i,k}] = local_optim(i, x0, xs(:,i), us(:,i), Qi{i}, Ri{i}, N, ...
                                             param, z_Ni{i,l}, y_Ni{i,l}, rho, k);
...
function [w_Ni, vi] = local_optim(i, x0, xs_i, us_i, Qi, Ri, N, param, z_Ni, y_Ni, rho, k)
    persistent localOptimizer
    if k==2
        localOptimizer{i} = init_optimizer(xs_i, us_i, i, N,Qi,Ri, param, rho);
    end
    solutionSet = localOptimizer{i}(x0, z_Ni.x_Ni, y_Ni.x_Ni);
